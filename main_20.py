...
# 【修正】調整繪圖邊界留白空間，預留更多空間給放大的標題與座標文字
LEFT_MARGIN, RIGHT_MARGIN = 0.10, 0.95
TOP_MARGIN, BOTTOM_MARGIN = 0.88, 0.12  
...
def render_trip_curve_pil(stage_configs, default_colors, selected_idx=0, test_current=None, fig_w=480, fig_h=310, scale=2):
    ...
    # 【需求1修正】顯著加大字型尺寸以利手機顯示
    font_title = get_font(int(20 * scale))
    font_label = get_font(int(16 * scale))
    font_small = get_font(int(13 * scale))
    ...
    title_y_pos = max(plot_y0 - int(28 * scale), int(6 * scale))
    
    draw.text((plot_x0, title_y_pos), "Schneider Electric (Taiwan)", fill="#000000", font=font_title)
    draw.text((plot_x1 - int(200 * scale), title_y_pos), f"Base Voltage : {v_base_str}", fill="#D90429", font=font_title)
    ...
    for dec in range(1, 6):
        ...
        for sub in range(1, 10):
            ...
            if is_major and px <= plot_x1:
                ...
                draw.text((px - int(12 * scale), plot_y1 + int(6 * scale)), label, fill="#333333", font=font_label)
    ...
    for y_val in y_ticks:
        ...
        draw.text((plot_x0 - int(48 * scale), py - int(8 * scale)), f"{y_val:g}", fill="#333333", font=font_label)
    ...
    # X 軸標籤文字位置與字型適度下移與放大
    draw.text((plot_x0 + (plot_x1 - plot_x0) // 2 - int(40 * scale), plot_y1 + int(24 * scale)), "Current (A)", fill="#333333", font=font_label)
...
def main(page: ft.Page):
    ...
    chart_dim = {"w": 480, "h": 280} # 可將預設 h 調小（例如 260）
    ...
    async def reset_chart_scale(e):
        await chart_interactive.reset_async()
        page.update()
    ...
    # 【需求2修正】修改螢幕旋轉與 Resize 邏輯
    def on_page_resize(e):
        pw = page.width if page.width else 360
        ph = page.height if page.height else 800

        is_landscape = pw > ph
        new_w = max(int(pw - 16), 280)  # 讓橫置時全寬展開

        if is_landscape:
            # 橫屏時寬度拉滿，高度動態根據畫面高度計算（最高不超過螢幕高度的 60%）
            new_h = min(int(ph * 0.6), int(new_w * 0.55))
        else:
            # 直屏時維持適當的寬高比例與最高限制
            new_h = int(new_w / 1.35)    # 調大分母，縮小高寬比
            new_h = min(new_h, 250)   # << 修改此處：改為 min(new_h, 250) 可縮小圖表，騰出下方空間
